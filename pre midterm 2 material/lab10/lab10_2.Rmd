...
```{r}
p+ geom_jitter(size=3) #geompoint but with random noise that an help wth over plotting
```
...
```{r}
p+ geom_point(aes(color=thermoregulation, shape= thermoregulation), size=2)
```

What about just filling in with a color preference or size?
```{r}
p+ geom_point(size=3, color= "black", alpha=.4, shape=21, fill="thistle") 
...
homerange %>%
  ...
  geom_bar()+
  coord_flip()+
  labs(title="Observations per taxonomic group",
       x="taxon",
       y=NULL) #take away x axis label
```
