...
elephants %>%
  ...
  summarise(estimated_age_years=mean(estimated_age_years, na.rm= T), 
...
elephants %>%
  group_by(sex, years_of_sample_collection) %>%
  summarise(median_age_years=median(estimated_age_years, na.rm= T), 
...
elephants %>%
  group_by(sex, years_of_sample_collection) %>%
  summarise(sd_age_years=sd(estimated_age_years, na.rm= T), 
...
msleep %>%
  group_by(order) %>%
  summarise(mean_brain_weight=mean(brainwt, na.rm= T), 
...
# brainwt is NA for all 3 samples of Cetacea, either not measured or dont have a brain.
```
