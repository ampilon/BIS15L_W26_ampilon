...
```{r}
q <- homerange %>%
  ggplot(aes(x= log10_mass, y= log10_preymass, colour = thermoregulation))+
  geom_point()+
...
```{r}
p+theme_linedraw()+
  theme(legend.position = "bottom", #moving the legend can say left or right top or bottom
...
homerange %>% 
  ...
  geom_boxplot()+
  facet_wrap(~trophic_guild~., ncol=2, labeller=label_both)+ #can also be nrow
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
