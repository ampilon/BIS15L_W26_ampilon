...
gap %>%
  ...
  labs(title= "GDP per capita in USA, 2000-2025",
       x= "Year",
       y="GDP")
```
...
gap %>%
  filter(year== "1935") %>%
  ggplot(aes(x=lex))+
  geom_density(color="black", fill= "turquoise4", alpha = .3)+
  labs(title = "Distribution of Life Expectancy")
```
...
gap %>%
  filter(year== "1935") %>%
  ggplot(aes(x=lex))+
  geom_histogram(aes(y=after_stat(density)),color="black", fill= "turquoise4", bins= 30)+
...
```{r}
income_cat2000 %>%
  drop_na(income_category) %>% #get rid of NA
  ggplot(aes(x=income_category, fill = income_category))+
  geom_bar()+
  labs(title = "Countries in each income category")
```
