...
```{r}
ggmap(mapshark) +
  geom_point(data = sharks, aes(longitude, latitude), size = 0.5, color = "red3", alpha = 0.8)+
...
ggmap(mapshark) +
  geom_point(data = fatal_sharks, aes(longitude, latitude), size = 0.5, color = "red3", alpha = 0.8)+
  labs(x = "Longitude", y = "Latitude", title = "Shark Attacks in California")
```
...
leaflet(SD_sharks) %>%
  addProviderTiles(providers$Terrain) %>%
  addCircleMarkers(
    lng = ~longitude,
    lat = ~latitude,
    radius = 2,
