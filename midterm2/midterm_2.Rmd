...
```{r}
polar_bears_w_years <- polar_bears %>%
    separate(
    capture_date,
...
```{r}
 #polar_bears_w_years %>%
  ...
  #ggplot(aes(x = year, y =  count(year)) )+
  #geom_line()+
  #theme(axis.text.x = element_text(angle=90, hjust = 1))+
  #labs(title= "Samples per year by population",
       #x= "Year",
       #y="Samples")
# plot can be made many ways, both pop on same plot, or faseted as two seperate plots
```
5. (2 points) What are the min, median, mean, and max values for `urea_nitrogen` and `creatinine`?
```{r}
polar_bears_w_years %>%
  summarise( min_u = min(urea_nitrogen),
             max_u = max(urea_nitrogen),
             mean_u = mean(urea_nitrogen),
             min_c = min(creatinine),
             max_c = max(creatinine),
             mean_c = mean(creatinine))
...
```{r}
pb_w_un_creat_ratio <- polar_bears_w_years %>%
  reframe(un_creat_ratio = ((urea_nitrogen*.466)/creatinine))
```
...
```{r}
pb_fasting <- pb_w_un_creat_ratio %>%
...
```{r}
#pull out fastig=ng bears

#pb_fasting %>%
  #group_by(popn) %>%
  #ggplot(aes(x = year, y =  count(fasting))) +
  #geom_line()+
  #theme(axis.text.x = element_text(angle=90, hjust = 1))+
  #labs(title= "Fasting per year by population",
       #x= "Year",
       #y="Fasting")
```
...
```{r}
mappb <- get_stadiamap(bbox, maptype = "stamen_terrain", zoom = 7)
```

```{r}
ggmap(mappb) +
  geom_point(data = polar_bears_w_years, aes(long, lat, color=popn), size = 0.5, , alpha = 0.8)+
...
#ui <- dashboardPage(
  ...
  #dashboardHeader(title = "Ratio by Population"),
...
#server <- function(input, output, session) {
 ...
 #output$plot <- renderPlot({
   ...
   #pb_fasting %>%
   #   ggplot(aes(x= popn, y=un_creat_ratio ))+
    #  geom_boxplot(fill = .data[[input$y]],
  #                 color = "black") +
  #    theme_classic()    
