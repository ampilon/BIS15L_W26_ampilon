...
```{r}
 polar_bears %>%
  count(popn, year) %>%
  ggplot(aes(x = as.numeric(year), y = n, group = popn, color= popn)) +
  geom_line() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "Samples per year",
       x = "Year",
       y = "Samples")
...
```{r}
#pull out fasting bears

polar_bears %>%
  filter(fasting == "T") %>%
  count(popn, year, fasting) %>%
  ggplot(aes(x = year, y =  n, group = popn, color = popn)) +
  geom_line()+
  facet_wrap(~popn)+
  theme(axis.text.x = element_text(angle=90, hjust = 1))+
...
```{r}
ggmap(mappb) +
  geom_point(data = polar_bears, aes(long, lat, color=popn), size = 1, , alpha = 0.8)+
...
server <- function(input, output, session) {
 ...
 output$plot <- renderPlot({
   ...
   polar_bears %>%
      ggplot(aes(x= popn, y=un_creat_ratio, fill = .data[[input$y]]))+
