...
#the average length of time that each mammal sleeps is 10.4 hours
```
...
```{r}
ggplot(data=msleep,mapping= aes(x=vore, y=bodywt))+geom_boxplot(na.rm=T)+scale_y_log10()
```
...
```{r}
ggplot(data=msleep,
       mapping=aes(x=bodywt, y=sleep_total)) + geom_point()+
  labs(title="body weight vs total sleep time", x="body weight", y="sleep total")
# When body weight is low sleep time is high. higer bodyweights have lower sleep times
```

**8. Which vore type tends to sleep the most? Build a plot to support your answer.**
```{r}
#insecti sleeps the most
ggplot(data=msleep,mapping= aes(x=vore, y=sleep_total))+geom_boxplot()+labs(title="total sleep times by vore", x="vore", y="sleep total")
```

**9. Is there a relationship between brain weight and body weight? Build a plot to support your answer.**
```{r}
#yes, as brain weight goes up do does body weight
ggplot(data=msleep,
       mapping=aes(x=brainwt, y=bodywt)) + geom_point()+
...
```{r}
ggplot(data=msleep,mapping= aes(x=order, y=bodywt))+geom_boxplot()+labs(title="body weight by order", x="order", y="body weight")+scale_y_log10()
```
