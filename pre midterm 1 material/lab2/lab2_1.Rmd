...
```{r}
z=10 # works but makes problems ahhhh
```
...
```{r}
mean_speed <- mean(c(venom_GT,chiron,veyron))
```
...
missing values mean you can not run the code

4. Recalculate the mean using the following code chunk. Why is the useful?  
```{r}
mean(new_vector, na.rm = TRUE) #removes NA values
```
useful because then if we are missing a data point we cans still run our data