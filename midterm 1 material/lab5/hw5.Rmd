...
# The structure is Rows: 569 and Columns: 24 and it does have NA's
```
...
#there are more carnivores represented
```
...
```
Hipoliti and partitus have the smallest homerange size and tarandus and africana have the largest.
...
homerange %>%
  filter( trophic.guild == "carnivore", 
       taxon== "mammals") %>%
  select(mean.hra.m2, common.name) %>%
   slice_max(mean.hra.m2, n=2)
```
Cheetah and lepord are the carnivorous mammals with the largest homeranges.
...
ggplot(data=homerange,
       mapping=aes(x=mean.mass.g, y=mean.hra.m2)) + geom_point()+
...
```{r}
data_birds_mammals <- filter(homerange, taxon == "birds"| taxon == "mammals")
ggplot(data=data_birds_mammals,
       mapping=aes(x=taxon, y=mean.hra.m2)) + geom_point()+
  labs(title="homerange sizes of birds and mammals", x="taxon", y="homerange size") +scale_y_log10()
```
...
```{r}
# What are the ten fish witht he smallest homerange?
homerange %>%
  filter( taxon== "marine fishes") %>%
  select(common.name, mean.hra.m2, taxon) %>%
