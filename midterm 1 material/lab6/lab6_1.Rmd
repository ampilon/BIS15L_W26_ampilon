...
```{r}
mammals%>%
  mutate(across(c("order", "family", "Genus"), tolower)) #specific coulums
```

This will change all columns to lower case. But, notice what happens to numeric columns.
```{r}
mammals%>%
  mutate(across(everything(), tolower)) #changed numerics to characters :(
...
```{r}
mammals%>%
  mutate(across(everything(where.is(character())), tolower)) 
```
